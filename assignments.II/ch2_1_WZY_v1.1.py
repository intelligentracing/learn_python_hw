...
def correlation_tracking(save_result,tracker):
    #读取文件
    ...
    #初始化tracker
    #每次更新tracker都会导致tracker变化,所以一开始先创建一个,后续若能使用就不再更新
    ...
    while (grabber.isOpened()): 
        ...
        if trackingStatus == LOST_TRACK:
            ...
            if len(faces)>0:
                ...
                status = temp_tracker.init(frame, (x,y, w, h))
                #如果监测初始化成功
                if status:
                    #将状态设置为更新
        ...
        # 若状态为更新
            ...
            #获取更新后的状态
            status, boundingBox = temp_tracker.update(frame)
            #如果获取成功
            if status:
                #h画出更新后人脸的位置
                x, y, w, h = boundingBox
    ...
    print("the time cost of "+str(tracker)+" is: "+str(time_elapsed))
    print("the number of losing frame is: "+str(break_frame))
    print("the number of losing track is: "+str(losing_track))
