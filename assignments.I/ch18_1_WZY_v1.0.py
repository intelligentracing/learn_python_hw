...
def get_learning_rate(t):
    learningrate=[0.2,0.3,0.4,0.5,0.1]
    return learningrate[t%len(learningrate)]
...
def onclick(event):
    ...
    for i in range (times):
        epsilon = 0.001
        learn_rate = get_learning_rate(i)
        delta = np.inf
        xlist = [event.xdata]  
        ...
        while delta > epsilon and running_time<1000:
            ...
            running_time+=1
        if len(xlist)<len(gxlist) or i==0:
            gylist=ylist
            gxlist=xlist
            best_learningrate=learn_rate
