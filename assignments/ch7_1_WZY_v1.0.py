...
def merge_sort(input_list,reverse=False):
    ...
    if len(input_list)>1:
        ...
        j=0
        k=0
        boli=bool(type(left_half[i])!=int and type(left_half[i])!=float)
        bolj=bool(type(right_half[j])!=int and type(right_half[j])!=float)
        bolk=bool(type(input_list[k])!=int and type(input_list[k])!=float)
    
        if boli==1 or bolj==1 or bolk==1:
            return False
